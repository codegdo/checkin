  ...
  BEGIN
    ...
    IF _userid IS NULL THEN
      --INSERT
        ...
        INTO data
        FROM (
          SELECT * FROM u LEFT JOIN c on c.id = u.contact_id
        ) r;
    ELSE
      --UPDATE
    END IF;
...
CALL sec.pr_user_save('[{"id":1, "value":"gdo3"},{"id":2, "value":"123"}]'::json, null::jsonb);

DROP PROCEDURE IF EXISTS sec.pr_user_save(json, jsonb);
