...
CREATE TABLE IF NOT EXISTS _sys.migration (
  ...
  FOREIGN KEY (migration_category_id) REFERENCES _sys.migration_category (id) ON DELETE CASCADE
);
...
BEGIN
  ...
  IF database_initialization_id IS NOT NULL THEN
    ...
    INSERT INTO _sys.migration (migration_category_id, name, description, execution_order, app_version) VALUES
