CREATE OR REPLACE FUNCTION org.fn_get_form_type_field(input_form_type_id INT)
RETURNS TABLE (
  field_id INT,
  field_type VARCHAR,
...
BEGIN
  ...
  FROM
    ...
    JOIN org.field fld ON fto.object_id = fld.object_id
    LEFT JOIN org.field_default fd ON fld.id = fd.field_id
    LEFT JOIN org.form_field ff ON fld.id = ff.field_id
  WHERE
...
--SELECT * FROM org.fn_get_form_type_field(1);
