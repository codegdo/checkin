...
CREATE OR REPLACE FUNCTION sec.fn_generate_account_id()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  LOOP
    ...
    EXIT WHEN NOT EXISTS (SELECT 1 FROM sec.account WHERE account_id = new_account_id);
  END LOOP;
...
BEFORE INSERT ON sec.account
FOR EACH ROW
EXECUTE FUNCTION sec.fn_generate_account_id();
