...
CREATE OR REPLACE FUNCTION sys.fn_get_migration_previous()
RETURNS sys.migration AS $$
DECLARE
  last_migration sys.migration;
  previous_migration sys.migration;
...
BEGIN
  ...
  ELSE
    ...
    SELECT * INTO previous_migration FROM sys.migration
    WHERE sys.migration.execution_order = previous_execution_order
    AND sys.migration.is_executed = TRUE;
  END IF;
...
-- SELECT * FROM sys.fn_get_previous_migration();
