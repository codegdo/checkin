...
CREATE OR REPLACE FUNCTION sys.fn_get_migration_next()
RETURNS sys.migration AS $$
DECLARE
  last_migration sys.migration;
  next_migration sys.migration;
...
BEGIN
  ...
  SELECT * INTO last_migration FROM sys.migration
  WHERE sys.migration.is_executed = TRUE
  ...
  IF last_migration IS NULL THEN
    SELECT * INTO next_migration FROM sys.migration
    WHERE sys.migration.is_executed = FALSE
  ...
  ELSE
    ...
    SELECT * INTO next_migration FROM sys.migration
    WHERE sys.migration.execution_order = next_execution_order
    AND sys.migration.is_executed = FALSE;
  END IF;
...
-- SELECT * FROM sys.fn_get_next_migration();
