...
CREATE TABLE IF NOT EXISTS sys.migration_script (
    ...
    FOREIGN KEY (migration_rollback_id) REFERENCES sys.migration_rollback (id) ON DELETE SET NULL
);
...
BEGIN
  ...
  IF setup_public_functions_id IS NOT NULL THEN
    ...
    FROM sys.migration_rollback mr
    WHERE mr.id = setup_public_functions_id;

    -- Insert rolback 'Setup Public Functions'
    INSERT INTO sys.migration_script (migration_id, migration_rollback_id, name, description, database_name, schema_name, object_type, script_type, script_path, script_order) VALUES
