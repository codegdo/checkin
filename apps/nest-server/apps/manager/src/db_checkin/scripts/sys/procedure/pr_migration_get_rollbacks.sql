/*
  ...
  @param {JSON} result
 */
CREATE OR REPLACE PROCEDURE sys.pr_migration_get_rollbacks(
  IN migration_id INT,
  OUT result JSON
)
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT COALESCE(
      (SELECT json_agg(migration_data) FROM sys.fn_get_migration_scripts_next(migration_id) AS migration_data),
      '[]'
