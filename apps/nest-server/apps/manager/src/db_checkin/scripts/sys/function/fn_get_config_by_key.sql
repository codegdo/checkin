...
CREATE OR REPLACE FUNCTION sys.fn_get_config_by_key(input_key VARCHAR)
RETURNS TABLE (
  key_exists BOOLEAN,
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM sys.config WHERE name = input_key) THEN
...
-- SELECT * FROM sys.fn_get_config_key('ENABLE_DROP_DB_SYSTEM_CONFIG');