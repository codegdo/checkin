/*
  Author: Giang Do
  Date: 09/18/2023
  Description: This stored procedure updates the sys.migration table with migration
  execution details and inserts an audit record into the sys.migration_audit table.
 ...
 */
CREATE OR REPLACE PROCEDURE sys.pr_migration_update_complete(
  IN payload JSON,
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM sys.migration WHERE id = migration_id) THEN
  ...
  -- INSERT INTO sys.migration_audit (migration_id, updated_at, updated_by)
