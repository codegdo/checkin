-- Create the '_org.form_field' table
CREATE TABLE IF NOT EXISTS _org.form_field (
  ...
  FOREIGN KEY(form_id) REFERENCES _org.form(id),
  FOREIGN KEY(field_id) REFERENCES _org.field(id)
);

-- Create a trigger that fires on UPDATE and calls the function
CREATE TRIGGER form_field_update_trigger
BEFORE UPDATE ON _org.form_field
FOR EACH ROW
EXECUTE FUNCTION fn_updated_at();

-- Insert data into the 'form_field' table
INSERT INTO _org.form_field (form_id, field_id, title, translation, options, is_required) VALUES
