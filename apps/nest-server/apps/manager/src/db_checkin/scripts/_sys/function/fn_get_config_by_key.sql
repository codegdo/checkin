...
CREATE OR REPLACE FUNCTION _sys.fn_get_config_by_key(input_key VARCHAR)
RETURNS TABLE (
  key_exists BOOLEAN,
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM _sys.config WHERE name = input_key) THEN
...
-- SELECT * FROM _sys.fn_get_config_key('ENABLE_DROP_DB__sysTEM_CONFIG');