/*
  ...
  @param {JSON} result
 */
CREATE OR REPLACE PROCEDURE _sys.pr_migration_get_scripts(
  IN migration_id INT,
  OUT result JSON
)
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT COALESCE(
    (
      ...
      FROM _sys.fn_get_migration_scripts_for_execution_next(migration_id) AS migration_data
    ), '[]'
  ...
  IF migration_scripts IS NOT NULL THEN
    ...
    SELECT COALESCE(
      (
        ...
        FROM _sys.fn_get_migration_rollbacks_for_execution_next(migration_id) AS rollback_data
      ), '[]'
...
-- Example usage:
-- CALL _sys.pr_migration_get_scripts(1, null);
