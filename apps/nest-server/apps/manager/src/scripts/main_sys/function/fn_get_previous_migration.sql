...
DECLARE
  ...
  previous_migration main_sys.migration;
...
BEGIN
  ...
  ELSE
    ...
    SELECT * INTO previous_migration FROM main_sys.migration
    WHERE main_sys.migration.execution_order = previous_execution_order
    AND main_sys.migration.is_executed = TRUE;
  END IF;
...
-- SELECT * FROM main_sys.fn_get_previous_migration();
