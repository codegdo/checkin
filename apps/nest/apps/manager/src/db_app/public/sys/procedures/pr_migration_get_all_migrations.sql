/*
  ...
  @param {JSON} result
 */
CREATE OR REPLACE PROCEDURE pr_migration_get_all_migrations(
  OUT result JSON
)
AS $$
DECLARE
...
BEGIN
  ...
  INTO migration_data
  FROM fn_migration_get_all_migrations() AS migrations;
...
BEGIN
  ...
  REVOKE EXECUTE ON PROCEDURE pr_migration_get_all_migrations(json) FROM public;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'api_manager') THEN
    ...
    GRANT EXECUTE ON PROCEDURE pr_migration_get_all_migrations(json) TO api_manager;
  ELSE
