CREATE OR REPLACE FUNCTION migration_fn_get_scripts_for_migration(migrationId INT)
RETURNS TABLE (
  id INT,
  name VARCHAR,
  category VARCHAR,
...
REVOKE EXECUTE ON FUNCTION migration_fn_get_scripts_for_migration(integer) FROM public;

-- SELECT * FROM migration_fn_get_scripts_for_migration(1);