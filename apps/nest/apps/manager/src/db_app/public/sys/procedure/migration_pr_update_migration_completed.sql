 ...
 */
CREATE OR REPLACE PROCEDURE migration_pr_update_migration_completed(
  IN payload JSON,
  IN updatedBy TEXT
)
AS $$
...
BEGIN
  ...
  REVOKE EXECUTE ON PROCEDURE migration_pr_update_migration_completed(json, text) FROM public;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'api_manager') THEN
    ...
    GRANT EXECUTE ON PROCEDURE migration_pr_update_migration_completed(json, text) TO api_manager;
  ELSE
