/*
  ...
  @param {JSON} result
 */
CREATE OR REPLACE PROCEDURE pr_migration_get_rollbacks_for_migration(
  IN migrationId INT,
  OUT result JSON
)
AS $$
...
BEGIN
  ...
  SELECT COALESCE(
      (SELECT json_agg(migration_data) FROM fn_migration_get_rollbacks_for_migration(migrationId) AS migration_data),
      '[]'
...
BEGIN
  ...
  REVOKE EXECUTE ON PROCEDURE pr_migration_get_rollbacks_for_migration(int, out json) FROM public;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'api_user') THEN
    ...
    GRANT EXECUTE ON PROCEDURE pr_migration_get_rollbacks_for_migration(int, out json) TO api_user;
  ELSE
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'api_manager') THEN
    ...
    GRANT EXECUTE ON PROCEDURE pr_migration_get_rollbacks_for_migration(int, out json) TO api_manager;
  ELSE
