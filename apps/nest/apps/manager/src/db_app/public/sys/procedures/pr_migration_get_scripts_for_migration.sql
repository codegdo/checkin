/*
  ...
  @param {JSON} result
 */
CREATE OR REPLACE PROCEDURE pr_migration_get_scripts_for_migration(
  IN migrationId INT,
  OUT result JSON
)
AS $$
...
BEGIN
  ...
  SELECT COALESCE(
    (
      ...
      FROM fn_migration_get_scripts_for_migration(migrationId) AS migration_data
    ), '[]'
  ) INTO scripts;
  ...
  IF scripts IS NOT NULL THEN
    ...
    SELECT COALESCE(
      (
        ...
        FROM fn_migration_get_rollbacks_for_migration(migrationId) AS rollback_data
      ), '[]'
...
BEGIN
  ...
  REVOKE EXECUTE ON PROCEDURE pr_migration_get_scripts_for_migration(int, out json) FROM public;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'api_user') THEN
    ...
    GRANT EXECUTE ON PROCEDURE pr_migration_get_scripts_for_migration(int, out json) TO api_user;
  ELSE
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'api_manager') THEN
    ...
    GRANT EXECUTE ON PROCEDURE pr_migration_get_scripts_for_migration(int, out json) TO api_manager;
  ELSE
...
-- Example usage:
-- CALL pr_migration_get_scripts_for_migration(1, null);
