CREATE OR REPLACE PROCEDURE system_pr_drop_tables(
  tableNames TEXT[] DEFAULT '{}'
)
AS $$
BEGIN
  -- Execute the function
  PERFORM system_fn_drop_tables(tableNames);
END;
...
BEGIN
  ...
  REVOKE EXECUTE ON PROCEDURE system_pr_drop_tables(text[]) FROM public;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'api_manager') THEN
    ...
    GRANT EXECUTE ON PROCEDURE system_pr_drop_tables(text[]) TO api_manager;
  ELSE
