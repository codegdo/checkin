CREATE OR REPLACE PROCEDURE _pg_execute_script(
  scriptContent text,
...
BEGIN
  ...
  REVOKE EXECUTE ON PROCEDURE _pg_execute_script(text, text, int) FROM public;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'api_manager') THEN
    ...
    GRANT EXECUTE ON PROCEDURE _pg_execute_script(text, text, int) TO api_manager;
  ELSE
