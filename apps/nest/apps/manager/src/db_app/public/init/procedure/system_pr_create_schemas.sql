CREATE OR REPLACE PROCEDURE system_pr_create_schemas(
  schemaNames TEXT[] DEFAULT '{}'
)
AS $$
BEGIN
  -- Execute the function
  PERFORM system_fn_create_schemas(schemaNames);
END;
...
BEGIN
  ...
  REVOKE EXECUTE ON PROCEDURE system_pr_create_schemas(text[]) FROM public;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'api_manager') THEN
    ...
    GRANT EXECUTE ON PROCEDURE system_pr_create_schemas(text[]) TO api_manager;
  ELSE
