CREATE OR REPLACE FUNCTION util_fn_get_granted_privileges_by_user(user_name text)
RETURNS TABLE (
  obj_schema text,
  obj_type text,
  obj_name text,
...
REVOKE EXECUTE ON FUNCTION util_fn_get_granted_privileges_by_user(text) FROM public;

-- SELECT * FROM util_fn_get_granted_privileges_by_user('api_manager_user');