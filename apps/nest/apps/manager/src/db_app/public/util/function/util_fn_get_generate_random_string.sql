...
CREATE OR REPLACE FUNCTION util_fn_get_generate_random_string(length INT, prefix VARCHAR DEFAULT NULL)
RETURNS VARCHAR AS $$
DECLARE
...
REVOKE EXECUTE ON FUNCTION util_fn_get_generate_random_string(integer, varchar) FROM public;

-- Example 1: Generate a random string of length 8 without a prefix
--SELECT util_fn_get_generate_random_string(8);

-- Example 2: Generate a random string of length 10 with a prefix
--SELECT util_fn_get_generate_random_string(10, 'ID-');
