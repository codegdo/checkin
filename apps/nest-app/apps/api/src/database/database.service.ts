...
export class DatabaseService {
  ...
  async seedSchemas(): Promise<Observable<{message: string}>> {
    ...
    try {
      return this.migrationService.send('db_seed_schemas', {}).pipe(
  ...
  async dropSchemas(): Promise<Observable<{message: string}>>  {
    const isDropActionEnabled = this.configService.get<string>('DB_DROP_ACTION_IS_ENABLED') === 'true';
    ...
    try {
      return this.migrationService.send('db_drop_schemas', {}).pipe(
        map((response: {message: string}) => response)
      );
    } catch(error) {
      console.log(error);
  ...
  async seedMigrations(): Promise<Observable<{message: string}>> {
    try {
      return this.migrationService.send('db_seed_migrations', {}).pipe(
        map((response: {message: string}) => response)
      );
    } catch (error) {
      console.error(error);
  ...
  async dropMigrations(): Promise<Observable<{message: string}>>  {
    try {
      return this.migrationService.send('db_drop_migrations', {}).pipe(
        map((response: {message: string}) => response)
      );
    } catch(error) {
      console.log(error);
