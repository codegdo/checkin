...
export const NumPadKey: React.FC<NumPadKeyProps> = ({ className = 'numpad-key' }): JSX.Element => {
  ...
  const { counter = 0, digit, autoSubmit, handleClick } = context;
  ...
  return <div className={className}>
    ...
    <div>
      {
        <button className={counter > 0 ? '' : 'invisible'} type="button" name="clear" onClick={e => handleClick(e)}>delete</button>
      }
      <button type="button" name="key" value="0" onClick={e => handleClick(e)}>0</button>
      {
        <button className={autoSubmit ? 'invisible' : (counter == digit ? '' : 'invisible')} type="button" name="enter" onClick={e => handleClick(e)}>enter</button>
      }
